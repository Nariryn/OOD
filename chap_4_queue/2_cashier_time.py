...
for i in range(1, int(time)+1):
    if time_1 == 3 and not secon_queue.is_empty():
        secon_queue.dequeue()
        time_1 = 0
    if time_2 % 2 == 0 and not last_queue.is_empty():
        last_queue.dequeue()
    if len(secon_queue.get_item()) < 5 and not first_queue.is_empty():
        secon_queue.enqueue(first_queue.dequeue())
    else:
    ...
    if not last_queue.is_empty():
        time_2 += 1
    print(f"{i} {first_queue.get_item()} {secon_queue.get_item()} {last_queue.get_item()}")
